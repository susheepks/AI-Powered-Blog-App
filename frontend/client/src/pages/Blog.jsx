  ...
  return data ? (
    <div className='relative'>
      ...
      <div className='text-center mt-20 text-gray-600'>
        ...
        font-semibold max-w-2xl mx-auto text-gray-800' >{data.title}</h1>
        <h2 className='my-5 max-w-lg truncate mx-auto'>{data.subTitle}</h2>
        <p className='inline-block py-1 px-4 rounded-full mb-6 border text-sm
        border-primary/35 bg-primary/5 font-medium text-primary'>Micheal Brown</p>
      </div>
      ...
      <div className='mx-5 max-w-5xl md:mx-auto my-10 mt-6'>
            ...
            {comments.map((item,index)=>(
              <div key={index} className='relative bg-primary/2 border
               border-primary/5 max-w-xl p-4 rounded text-gray-600'>
                ...
                <div className='absolute right-4 bottom-3 flex 
                items-center gap-2 text-xs'>{Moment(item.createdAt).fromNow()}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className='max-w-3xl mx-auto'>
          ...
          <form action="" onSubmit={addcomment} className='flex flex-col items-start gap-4
          max-w-lg'>

          <input onChange={(e)=>setName(e.target.value)} value={name} 
          type="text" placeholder='name' required className='w-full
          p-2 border mb-2 border-gray-300 rounded outline-none' />
